...
def validation_loss(model_bert, model_ef, neural, labels, loss_function):
	...
	for k in range(iters):
		...
		for i in range(8):
			...
			cap_1, cap_2 = get_pair_cap(idx, length, labels, i)
		...
		with torch.no_grad():
			anchor, positive, negative = neural(features, emb_batch_1.to(torch.device("cuda:0")), emb_batch_2.to(torch.device("cuda:0")))
